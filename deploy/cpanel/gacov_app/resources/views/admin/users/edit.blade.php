...
<div class="page-header">
    ...
    <p class="page-subtitle">
        <a href="{{ route('admin.users.index') }}" style="color:var(--gacov-text-muted);text-decoration:none">Usuarios</a>
        / {{ $user->name }}
    </p>
</div>
...
<div class="panel" style="max-width:860px">
    <div class="panel-header">
        <span class="panel-title">Datos del usuario</span>
        @if($user->last_login_at)
        <span style="font-size:12px;color:var(--gacov-text-muted);margin-left:auto">
            Último acceso: {{ $user->last_login_at->format('d/m/Y H:i') }}
            ({{ $user->last_login_at->diffForHumans() }})
        </span>
        @else
        <span style="font-size:12px;color:var(--gacov-text-muted);margin-left:auto">Nunca ha iniciado sesión</span>
        @endif
    </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-5)">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="phone" name="phone"
                           ...
                           value="{{ old('phone', $user->phone) }}" maxlength="20" placeholder="Ej: 3001234567">
                ...
                {{-- Confirmar contraseña (se muestra si se está cambiando) --}}
                <div class="form-group" id="confirm-field" style="display:none">
                ...
                <div class="form-group" id="route-field"
                     style="display:{{ (old('role', $user->getRoleNames()->first() ?? '') === 'conductor') ? 'block' : 'none' }}">
                    ...
                    <select id="route_id" name="route_id"
                        ...
                        <option value="{{ $route->id }}"
                            {{ old('route_id', $user->route_id) == $route->id ? 'selected' : '' }}>
                ...
                <div style="display:flex;flex-direction:column;gap:var(--space-4);justify-content:flex-end;padding-bottom:var(--space-2)">
                    ...
                    <div style="display:flex;align-items:center;gap:var(--space-3)">
                        ...
                        <input type="checkbox" id="must_change_password" name="must_change_password" value="1"
                               {{ old('must_change_password', $user->must_change_password ? '1' : '0') === '1' ? 'checked' : '' }}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Mostrar campo confirmación si hay errores de validación y volvió a la vista
    const passwordInput = document.getElementById('password');
