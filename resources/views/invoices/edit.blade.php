...
@section('title', "Editar Factura {$invoice->full_number}")
...
<div class="page-header">
    <div class="page-header-left">
        ...
        <h1>Editar Factura {{ $invoice->full_number }}</h1>
        <p class="page-subtitle">Solo se pueden editar facturas en estado borrador</p>
    </div>
</div>

<form action="{{ route('invoices.update', $invoice) }}" method="POST" id="invoice-form" class="invoice-form">
    @csrf
    @method('PUT')
                ...
                <div class="form-grid">
                    <div class="form-group full-width">
                        ...
                        <input type="text" name="client_name" id="client_name" 
                               value="{{ old('client_name', $invoice->client_name) }}" required class="form-input">
                    </div>
                    <div class="form-group">
                        <label for="client_nit">NIT / Cédula *</label>
                        <input type="text" name="client_nit" id="client_nit" 
                               value="{{ old('client_nit', $invoice->client_nit) }}" required class="form-input">
                    </div>
                    <div class="form-group">
                        <label for="client_phone">Teléfono</label>
                        <input type="text" name="client_phone" id="client_phone" 
                               value="{{ old('client_phone', $invoice->client_phone) }}" class="form-input">
                    </div>
                    <div class="form-group full-width">
                        <label for="client_address">Dirección</label>
                        <input type="text" name="client_address" id="client_address" 
                               value="{{ old('client_address', $invoice->client_address) }}" class="form-input">
                    </div>
                    <div class="form-group full-width">
                        <label for="client_email">Email</label>
                        <input type="email" name="client_email" id="client_email" 
                               value="{{ old('client_email', $invoice->client_email) }}" class="form-input">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="prefix" id="prefix" value="{{ old('prefix', $invoice->prefix) }}" 
                               maxlength="10" class="form-input" readonly>
                    </div>
                    <div class="form-group">
                        <label>Número</label>
                        <div class="form-display">{{ $invoice->full_number }}</div>
                    </div>
                    <div class="form-group">
                        <label for="issue_date">Fecha de Emisión *</label>
                        <input type="date" name="issue_date" id="issue_date" 
                               value="{{ old('issue_date', $invoice->issue_date->format('Y-m-d')) }}" required class="form-input">
                    </div>
                    <div class="form-group">
                        <label for="due_date">Fecha de Vencimiento</label>
                        <input type="date" name="due_date" id="due_date" 
                               value="{{ old('due_date', $invoice->due_date?->format('Y-m-d')) }}" class="form-input">
                    </div>
                    <div class="form-group">
                        <label for="tax_rate">Tasa IVA (%)</label>
                        <select name="tax_rate" id="tax_rate" class="form-input">
                            <option value="0" {{ $invoice->tax_rate == 0 ? 'selected' : '' }}>0%</option>
                            <option value="5" {{ $invoice->tax_rate == 5 ? 'selected' : '' }}>5%</option>
                            <option value="19" {{ $invoice->tax_rate == 19 ? 'selected' : '' }}>19%</option>
                        </select>
                    </div>
                    <div class="form-group full-width">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group full-width">
                        ...
                        <textarea name="notes" id="notes" rows="3" class="form-input">{{ old('notes', $invoice->notes) }}</textarea>
                    </div>
                    <div class="form-group full-width">
                        <label for="terms">Términos y Condiciones</label>
                        <textarea name="terms" id="terms" rows="2" class="form-input">{{ old('terms', $invoice->terms) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <table class="items-table" id="items-table">
                ...
                <tr class="item-row">
                    <td>
                        <input type="text" name="items[{{ $index }}][description]" 
                               value="{{ old('items.' . $index . '.description', $item->description) }}" class="item-description" required>
                        <input type="hidden" name="items[{{ $index }}][product_key]" 
                               value="{{ old('items.' . $index . '.product_key', $item->product_key) }}" class="item-product-key">
                    </td>
                    <td>
                        <input type="text" name="items[{{ $index }}][unit]" 
                               value="{{ old('items.' . $index . '.unit', $item->unit) }}" class="item-unit" maxlength="20">
                    </td>
                    <td>
                        <input type="number" name="items[{{ $index }}][quantity]" 
                               value="{{ old('items.' . $index . '.quantity', $item->quantity) }}" 
                               min="0.0001" step="0.01" class="item-quantity" required>
                    </td>
                    <td>
                        <input type="number" name="items[{{ $index }}][unit_price]" 
                               value="{{ old('items.' . $index . '.unit_price', $item->unit_price) }}" 
                               min="0" step="0.01" class="item-price" required>
                    </td>
                    <td>
                        <input type="number" name="items[{{ $index }}][discount_rate]" 
                               value="{{ old('items.' . $index . '.discount_rate', $item->discount_rate) }}" 
                               min="0" max="100" step="0.01" class="item-discount">
                    </td>
                    <td>
                        <input type="number" name="items[{{ $index }}][tax_rate]" 
                               value="{{ old('items.' . $index . '.tax_rate', $item->tax_rate) }}" 
                               min="0" max="100" step="0.01" class="item-tax">
                    </td>
                    <td class="item-total text-right">${{ number_format($item->total, 2) }}</td>
                    <td>
            ...
            <tfoot>
                <tr class="totals-row">
                    ...
                    <td class="text-right" id="subtotal-display">${{ number_format($invoice->subtotal, 2) }}</td>
                    <td></td>
                </tr>
                <tr class="totals-row">
                    <td colspan="5" class="text-right totals-label">Descuento:</td>
                    <td class="text-right" id="discount-display">-${{ number_format($invoice->discount_amount, 2) }}</td>
                    <td></td>
                </tr>
                <tr class="totals-row">
                    <td colspan="5" class="text-right totals-label">IVA:</td>
                    <td class="text-right" id="tax-display">${{ number_format($invoice->tax_amount, 2) }}</td>
                    <td></td>
                </tr>
                <tr class="totals-row total-final-row">
                    <td colspan="5" class="text-right totals-label">TOTAL:</td>
                    <td class="text-right total-final" id="total-display">${{ number_format($invoice->total, 2) }}</td>
                    <td></td>
                </tr>
            </tfoot>
        </table>
    ...
    <div class="form-actions">
        <a href="{{ route('invoices.show', $invoice) }}" class="btn btn-ghost">Cancelar</a>
...
@push('styles')
<style>
.back-link { display: inline-flex; align-items: center; gap: 6px; color: var(--amr-text-muted, #9CA3AF); font-size: 14px; text-decoration: none; margin-bottom: 8px; }
...
.form-input { padding: 10px 12px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: var(--amr-text-primary, #F9FAFB); font-size: 14px; width: 100%; }
...
.items-table th { font-size: 11px; font-weight: 600; color: var(--amr-text-muted, #9CA3AF); text-transform: uppercase; letter-spacing: 0.5px; padding: 12px 8px; background: rgba(255,255,255,0.03); text-align: left; }
.items-table td { padding: 8px; vertical-align: middle; }
.items-table input { width: 100%; padding: 8px 10px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; color: var(--amr-text-primary, #F9FAFB); font-size: 13px; }
...
.btn-remove-item { display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 6px; background: transparent; border: none; color: #9CA3AF; cursor: pointer; }
...
@media (max-width: 900px) { .form-layout { grid-template-columns: 1fr; } .form-grid { grid-template-columns: 1fr; } .form-group.full-width { grid-column: 1; } }
</style>
@endpush
