...
<div class="page-header">
    <h1 class="page-title">Editar máquina</h1>
    <p class="page-subtitle">
        <a href="{{ route('machines.index') }}" style="color:var(--gacov-text-muted);text-decoration:none">Máquinas</a>
        /
        <a href="{{ route('machines.show', $machine) }}" style="color:var(--gacov-text-muted);text-decoration:none">{{ $machine->name }}</a>
        / Editar
    </p>
</div>
    ...
    <div class="panel-body">
        <form method="POST" action="{{ route('machines.update', $machine) }}">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-5)">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="worldoffice_code"
                           ...
                           value="{{ old('worldoffice_code', $machine->worldoffice_code) }}"
                ...
                <div class="form-group">
                    ...
                    <select name="route_id" class="form-input {{ $errors->has('route_id') ? 'is-invalid' : '' }}">
                        ...
                        <option value="{{ $route->id }}" {{ old('route_id', $machine->route_id) == $route->id ? 'selected' : '' }}>
                            {{ $route->name }}
                ...
                <div class="form-group">
                    ...
                    <select name="operator_user_id" class="form-input {{ $errors->has('operator_user_id') ? 'is-invalid' : '' }}">
                        ...
                        <option value="{{ $operator->id }}" {{ old('operator_user_id', $machine->operator_user_id) == $operator->id ? 'selected' : '' }}>
                            {{ $operator->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group" style="display:flex;align-items:center;gap:var(--space-3);padding-top:var(--space-6)">
                    ...
                    <input type="checkbox" name="is_active" id="is_active" value="1"
                           {{ old('is_active', $machine->is_active ? '1' : '0') === '1' ? 'checked' : '' }}
            ...
            <div style="display:flex;gap:var(--space-3);margin-top:var(--space-6);padding-top:var(--space-6);border-top:1px solid var(--gacov-border)">
                ...
                <a href="{{ route('machines.show', $machine) }}" class="btn" style="width:auto;background:var(--gacov-bg-elevated);color:var(--gacov-text-primary)">Cancelar</a>
            </div>
        </form>
    </div>
</div>
